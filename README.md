...
# HAProxy + Keepalived: Redundancy and high availability
...
```shell
docker build -f docker/web/Dockerfile -t webserver .
docker build -f docker/lb/Dockerfile -t loadbalancer .
```

### Launch Web Servers

Launch two docker containers for the image `webserver` as web servers:
```
docker run -d --name webserver1 --hostname myweb1 -p 18080:80 -p 18443:443 webserver bash -c "top"
docker run -d --name webserver2 --hostname myweb2 -p 19080:80 -p 19443:443 webserver bash -c "top"
```
...
Launch two docker containers for the image `loadbalancer` as load balancers:
```
docker run -d --name loadbalancer1 --hostname mylb1 -p 28080:8080 -p 28443:8443 -p 28090:8090 bash -c "top" loadbalancer

docker run -d --name loadbalancer2 --hostname mylb2 -p 29080:8080 -p 29443:8443 -p 29090:8090 bash -c "top" loadbalancer

##docker run -it --name mylb1 --hostname mylb1 -p 28080:8080 -p ##28443:8443 -p 28090:8090 --sysctl net.ipv4.ip_nonlocal_bind=1 ##--privileged morningspace/lab-lb
```
